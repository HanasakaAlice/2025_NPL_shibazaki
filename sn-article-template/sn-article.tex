...
\section{Theoretical Analysis}\label{ch:theory}

This section presents generalization (estimation-error) bounds for TRADES in supervised and PU settings, and derives a sufficient condition on the number of unlabeled samples under which PU+TRADES achieves a tighter bound than supervised TRADES.
...
the input data $\boldsymbol{x}\in\mathcal{X}$ are bounded,
\[
...
In supervised learning, we use labeled positive (P) and negative (N) samples.
\begin{equation}
\begin{aligned}
\mathscr{X}_{\mathrm{P}} &= \{\boldsymbol{x}_i^{\mathrm{P}}\}_{i=1}^{n_{\mathrm{P}}}
...
In contrast, in PU learning we use positive and unlabeled (U) data.
\[
...
\subsection{Preliminaries}We summarize the basic tools used in the analysis, including Rademacher complexity, contraction inequalities, and McDiarmid's inequality.
...
\begin{tcolorbox}[
    ...
    fonttitle = \bfseries,
    breakable = false]
\begin{dfn}[Rademacher complexity]\begin{equation}
 \mathfrak{R}_{n, \nu}(\mathscr{G})
...
\begin{tcolorbox}[
    ...
    fonttitle = \bfseries,
    breakable = false]
\begin{lem}[Talagrand's contraction lemma]Let $S_n=\{\bm{x}_i\}_{i=1}^n \stackrel{\text{i.i.d.}}{\sim} \nu(\bm{x})$.
\begin{equation}
...
\subsubsection{Upper Bound on the Rademacher Complexity}
\begin{tcolorbox}[
    colback = white,
    colframe = black!60,
    boxrule = 0.35pt,
    fonttitle = \bfseries,
    breakable = false]
\begin{thm}[Upper bound on the Rademacher complexity]$\mathscr{G}=\{\bm{x}\mapsto \bm{w}^{\top}\bm{x}\mid \|\bm{w}\|_\infty \le W\}$
\begin{equation}
...
\begin{tcolorbox}[
    ...
    fonttitle = \bfseries,
    breakable = false]
\begin{lem}[Rademacher complexity under adversarial perturbations]$\mathscr{G}=\{\bm x\mapsto \bm w^\top\bm x:\ \|\bm w\|_p\le W\}$
\[
...
% \begin{proof}
% $\sup_{\|\bm w\|_p\le W}\frac{1}{n}\sum_{i=1}^n \sigma_i \bm w^\top(\bm x_i+\bm e_i)
% = W\Big\|\frac{1}{n}\sum_{i=1}^n \sigma_i(\bm x_i+\bm e_i)\Big\|_q$．
% $\mathbb{E}_\sigma\big\|\frac{1}{n}\sum_{i=1}^n \sigma_i\bm e_i\big\|_q
% \end{proof}
...
\begin{tcolorbox}[
    ...
    fonttitle = \bfseries,
    breakable = false]
\begin{lem}[Vector contraction]\begin{equation}
\begin{aligned}
...
\begin{tcolorbox}[
    ...
    fonttitle = \bfseries,
    breakable = false]
\begin{thm}[McDiarmid's inequality]\begin{equation}
...
\subsection{Estimation Error Bound for Supervised TRADES}We first derive a uniform deviation bound and an estimation-error bound for the supervised TRADES objective.



\paragraph{Supervised TRADES risk (population and empirical risks)}\begin{equation}
\begin{aligned}
...
\begin{tcolorbox}[
    ...
    fonttitle = \bfseries,
    breakable = false]
\begin{thm}[Estimation error bound for supervised TRADES]\label{thm:pntr-estimation}


\begin{itemize}
\item \textbf{(Bounded losses)}
\begin{itemize}
...
\item \textbf{(Lipschitz continuity of losses)}
...
\begin{tcolorbox}[
    ...
    fonttitle = \bfseries,
    breakable = false]
\begin{lem}[Uniform deviation bound for supervised TRADES]\label{lem:pntr-unif-jp}
\begin{equation}
\begin{aligned}
...
\subsection{Estimation Error Bound for uPU+TRADES}We extend the analysis to the uPU+TRADES objective, which combines unbiased PU risk estimation with the TRADES regularization.



\paragraph{uPU+TRADES risk (population and empirical risks)}\begin{align}
...
\begin{thm}[Estimation error bound for uPU+TRADES]\label{thm:uputr-estimation}


\begin{itemize}
\item \textbf{(Bounded losses)} There exist constants $C_\ell, C_{\mathrm{KL}}>0$ such that,
...
\item \textbf{(Lipschitz continuity of losses)} There exist constants $L_\ell, L_{\mathrm{KL}}>0$ such that,
\begin{itemize}
\end{itemize}
\end{itemize}
...
\begin{tcolorbox}[
    ...
    fonttitle = \bfseries,
    breakable = false]
\begin{lem}[Uniform deviation bound for uPU+TRADES]\label{lem:uputr-unif-jp}
...
\eqref{eq:uputr-unif-bound-jp} holds.
...
\subsection{Estimation Error Bound for nnPU+TRADES}We further analyze nnPU+TRADES, which enforces non-negativity to mitigate overfitting caused by negative empirical risk estimates.

\label{subsec:nnpu-trades-bound}


\paragraph{nnPU+TRADES risk (population and empirical risks)}\[
\psi(g,\bm x)
:=
...
\begin{tcolorbox}[
    ...
    fonttitle = \bfseries,
    breakable = false]
\begin{thm}[Estimation error bound for nnPU+TRADES]\label{thm:nnputr-estimation}
...
\begin{tcolorbox}[
    ...
    fonttitle = \bfseries,
    breakable = false]
\begin{lem}[Uniform deviation bound for nnPU+TRADES]\label{lem:nnputr-unif-jp}
For any $\delta>0$, with probability at least $1-\delta$
\footnotesize
\begin{align}
...
\clearpage
\subsection{Sufficient Condition on the Number of Unlabeled Samples for PU+TRADES to Outperform Supervised TRADES}Finally, we compare the derived bounds and obtain a sufficient unlabeled-sample size condition ensuring that PU+TRADES yields a tighter estimation-error bound than supervised TRADES.

\label{subsec:nu_condition_trades}


\paragraph{Rademacher complexity of a linear hypothesis class}\begin{align}
\mathfrak{R}_{n,\nu}(\mathscr G)
...
\paragraph{(1) Comparing PN+TRADES and uPU+TRADES}\begin{align}
...
\paragraph{Main result: a sufficient condition on the number of unlabeled samples}
\begin{tcolorbox}[
    colback = white,
    colframe = black,
    fonttitle = \bfseries,
    breakable = false]
\begin{thm}[Sufficient unlabeled-sample condition for PU+TRADES to outperform supervised TRADES]\label{thm:nu-threshold-trades}
We compare the estimation-error bounds in Theorems~\ref{thm:pntr-estimation}, \ref{thm:uputr-estimation}, and \ref{thm:nnputr-estimation}.
\begin{equation}
...
\end{equation}
The right-hand side of the PN+TRADES bound (Eq.~\eqref{eq:pntr-bound-simplified})
